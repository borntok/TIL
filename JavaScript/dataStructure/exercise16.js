...
function ChaningHashTable() {
  this.table = new Array(HASH_SIZE);
  this.length = 0;
}

// hashCode(): 해시 함수
ChaningHashTable.prototype.hashCode = function (key) {
  let hash = 0;
...
// clear(): 초기화
ChaningHashTable.prototype.clear = function () {
  this.table = new Array(HASH_SIZE);
  this.length = 0;
};

// size(): 크기 반환
ChaningHashTable.prototype.size = function () {
  return this.length;
};

// getBuffer(): 데이터 셋 반환
ChaningHashTable.prototype.getBuffer = function () {
  let array = [];
...
// print(): 데이터 셋 출력
ChaningHashTable.prototype.print = function () {
  for (let i = 0; i < this.table.length; i++) {
    if (this.table[i]) {
      let current = this.table[i].head;
